...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Compare different node anomaly score aggregations on the same checkpoint."
    )
    parser.add_argument(
        "--checkpoint",
        type=str,
...
def build_incident_segments(edge_index: torch.Tensor, edge_error: torch.Tensor, num_nodes: int) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
...
def summarize_method(
    ...
    if write_node_scores:
        ensure_dir(output_dir)
        node_scores_path = output_dir / "node_scores.tsv"
...
def evaluate_single_graph(
    ...
    for method_name, node_scores in score_methods.items():
        method_output_dir = window_output_dir / method_name
        method_summaries[method_name] = summarize_method(
            ...
            write_node_scores=write_node_scores,
        )
...
def main() -> None:
    ...
    run_name = args.run_name or f"score_aggs_{checkpoint_path.parent.name}_{datetime.utcnow().strftime('%Y%m%d_%H%M%S')}"
    ...
    print(f"[compare-score-aggs] checkpoint={checkpoint_path}", flush=True)
    print(f"[compare-score-aggs] device={device}", flush=True)
    print(f"[compare-score-aggs] graphs={[str(path) for path in graph_paths]}", flush=True)
    ...
    for graph_path in graph_paths:
        ...
        for method_name, method_summary in summary["methods"].items():
            print_method_summary(window_name, method_name, method_summary)

    with (run_dir / "comparison_summary.json").open("w", encoding="utf-8") as handle:
        json.dump(aggregate, handle, indent=2)

    print(f"[compare-score-aggs] finished. outputs -> {run_dir}", flush=True)
